...
{$MODE DELPHI}{$H+}
{$ENDIF}
unit DcefB.Dcef3.CefFilescheme;
{$R 'DcefB.Dcef3.CefFilescheme.res'}
{$WARN SYMBOL_PLATFORM OFF}

interface

uses DcefB.Dcef3.CefLib, Classes;
...
type
  TFileInfo = class
    ...
    date: TFileTime;
    attr: Integer;
    constructor Create(const aname: ustring; asize: Int64; adate: TFileTime;
      aattr: Integer);
...
constructor TFileInfo.Create(const aname: ustring; asize: Int64;
  adate: TFileTime; aattr: Integer);
begin
  name := aname;
  size := asize;
  date := adate;
...
const
  s: array [0 .. 5] of Int64 = (1024, 1048576, 1073741824, 1099511627776,
    1125899906842624, 1152921504606846976);
begin
  if size < s[0] then
    Result := Format('%d B', [size])
  else if size < s[1] then
    Result := Format('%0.1f kB', [size / s[0]])
  else if size < s[2] then
    Result := Format('%0.1f MB', [size / s[1]])
  else if size < s[3] then
    Result := Format('%0.1f GB', [size / s[2]])
  else if size < s[4] then
    Result := Format('%0.1f TB', [size / s[3]])
  else if size < s[5] then
    Result := Format('%0.1f PB', [size / s[4]])
  else
    Result := 'big';
end;
...
begin
  ...
  with sys do
    dt := EncodeDate(wYear, wMonth, wDay) + EncodeTime(wHour, wMinute, wSecond,
      wMilliSeconds);
...
begin
  if attr and faDirectory = faDirectory then
    ...
    d := 0;
  Result := Format('<script>addRow("%s","%0:s",%d,"%s","%s");</script>'#13#10,
    [Escape(name), d, DisplaySize, DisplayDate])
end;
...
begin
  ...
  else if (Item2.attr and faDirectory <> faDirectory) then
    Result := CompareText(Item1.name, Item2.name)
  else
    Result := 1;
end;
...
constructor TFileScheme.Create(const browser: ICefBrowser;
...
begin
  ...
  begin
    ...
    try
      ...
      if reg.OpenKey(ExtractFileExt(FPath), False) then
        FMimeType := reg.ReadString('Content Type')
      else if LowerCase(ExtractFileExt(FPath)) = '.pdf' then
  ...
  begin
    ...
    try
      ...
      begin
        repeat
          if rec.name <> '.' then
            Items.Add(TFileInfo.Create(rec.name, rec.size,
              rec.FindData.ftLastWriteTime, rec.attr));
  ...
  begin
    ...
    OutputUTF8
      ...
      'charset=UTF-8"/></head><body><h1>' + FPath +
      '</h1><h2>not found</h2></body></html>');
    FMimeType := 'text/html';
    FDataStream.Seek(0, soFromBeginning);
  end;
